...
const loan_reports = async (req, res, next) => {
    const branch_id = req.user.branch_id;
    try {
        await sequelize.query("SELECT installment_id, loan_id, amount, due_date, paid_date, getInstallmentStatus(installment_status) FROM loan_installment_banks left join loans l using(loan_id) WHERE l.branch_id = ?", { replacements: [branch_id] }).then(
...
const loan_reportss = async (req, res, next) => {
    const   branch_id = req.user.branch_id,
            current_month = date.format(new Date(), 'MM'); 

    try {
        await sequelize.query("SELECT installment_id, loan_id, amount, due_date, paid_date, installment_status  FROM loan_installment_banks left join loans l using(loan_id) WHERE l.branch_id = ?", { replacements: [branch_id] }).then(
            async (foundInstallments) => {
                if (foundInstallments[0].length != 0) {
                    req.foundInstallments = foundInstallments;
                    next();
